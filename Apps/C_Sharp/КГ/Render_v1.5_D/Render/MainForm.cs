    ...
    private void _ctrlCanvas_MouseWheel(object sender, MouseEventArgs e)
    {
      ...
      if (MainUnitProcessor.Model != null)
      {
        ...
        else
        {
          ...
          double coeff;
          double x = MyDrawing.XX(e.X);
          double y = MyDrawing.YY(e.Y);
          if (e.Delta < 0)
            coeff = 1.03;
          else
            coeff = 0.97;
          MyDrawing.x1p = x - (x - MyDrawing.x1p) * coeff;  //x1 > x
          MyDrawing.x2p = x + (MyDrawing.x2p - x) * coeff;
          MyDrawing.y1p = y - (y - MyDrawing.y1p) * coeff;
          MyDrawing.y2p = y + (MyDrawing.y2p - y) * coeff;
        }
      }
      DrawImage();
    }

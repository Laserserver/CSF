  ...
  public class ModelClass
  {
    ...
    private void GetCoeffs()
    {
      for (int i = 0; i < RLength; i++)
      {
        ...
        double[] TempVect = MatrixesAndVectors.Normalize(Camera);
        Polygons[i].CameraDist = 0.5 * Math.Abs(
          Polygons[i].Equation[0] * TempVect[0] +
          Polygons[i].Equation[1] * TempVect[1] +
          Polygons[i].Equation[2] * TempVect[2] +
        ...
        /* Polygons[i].CameraDist = MatrixesAndVectors.VectorGetLength(new double[3] {  //Camera.X, Polygons[i].Vector[1] - Camera.Y, Polygons[i].Vector[2] - Camera.Z }));
           Camera[0] - Points[Polygons[i].MaxZ - 1].X,
           Camera[1] - Points[Polygons[i].MaxZ - 1].Y,
           Camera[2] - Points[Polygons[i].MaxZ - 1].Z});*/
      }
    }
    ...
    private void GetMinZs()
    {
      for (int i = 0; i < RLength; i++)
      {
        Polygons[i].Equation = GetEquation(Polygons[i]);
      }
    }
    private double[] GetEquation(Structures.MyPolygon Pol)
    {
      Structures.MyPoint Z1 = Points[Pol.FP - 1];
      Structures.MyPoint Z2 = Points[Pol.SP - 1];
      Structures.MyPoint Z3 = Points[Pol.TP - 1];

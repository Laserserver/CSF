...
namespace YAISP1._8
{
  class ConHandler
  {
    ...
    {
      char Sw = ' ';
      ColorText("1. Добавить заголовок\n2. Добавить абзац.\n3. Добавить колонку.\n4. Добавить список.\n5. Закончить работу с текстом", 'a');
      try
      {
        Console.Write(">> ");
    ...
    private void _AddMult(int Type)
    {
      bool Temp = Type == 3;
      ColorText(Temp ? "Введите количество колонок (от 1 до 3): " : "Введите количество строк списка: ", 'i');
      int Num = 0; //Количество строк списка или количество колонок
      if (Temp)
      {
        if (Num > 1)
          CurrentDoc._HasMultipleColumns = true;
        if (Num > CurrentDoc._ColumnsCount)
          CurrentDoc._ColumnsCount = Num;   //У документа максимальное число колонок должно быть прописано
      }
      Console.Write(">> ");
      if (int.TryParse(Console.ReadLine(), out Num))
      {
        if (Num < 1 || (Temp && Num > 3))  //Обобщение интервала
        ...
        else
        {
          ...
          for (int i = 0; i < Num; i++)
          {
            ColorText("Введите " + (i + 1) + (Temp ? " колонку: " : " элемент списка: "), 'i');
            Console.Write(">> ");
            Arr[i] = Console.ReadLine();
          }
          CurrentDoc.AddDiv(Type, Arr);
        }
      }
      else
        ColorText("Введен недопустимый символ", 'e');
    ...
    public void PrintText()
    {
      if (CurrentDoc != null)
      {
        ...
        if (int.TryParse(Console.ReadLine(), out Width))
          if (Width < 1 || Width > 80)
            ColorText("Недопустимая ширина: " + Width, 'e');
          else
            ColorText(CurrentDoc.Format(Width), 'o');
        else
          ColorText("Введены недопустимые символы", 'e');
      }
      else

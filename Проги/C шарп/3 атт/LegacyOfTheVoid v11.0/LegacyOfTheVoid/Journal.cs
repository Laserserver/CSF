...
namespace ProjectRed
{
  class Journal
  {
    ...
    public static Journal Parse(string input)
    {
      ...
      List<ClassLesson> Subjects = GetLessonsList(New[1]);
      List<MarksForClass> StudMarks = new List<MarksForClass>();

      string[] Rows = New[2].Split('\n');
      string ClassName = null;
      List<LessonMarks> LessonType;
      int Pos = 2;
      string LessonName;    
      bool[] IsWas;
      string[,] StudMarksMatrix;
      string[] MiddleMarks;
      int LessonCount;
      ...
      for (int i = 0; i < ClassCount; i++)
      {
        ...
        for (int k = 0; k < LessonCount; k++)
        {
          ...
          Pos += TempL + 1; //21
          StudMarksMatrix = new string[int.Parse(Rows[Pos]), int.Parse(Rows[Pos + 1])];
          ...
          for (int l = 0; l < int.Parse(Rows[TempL]); l++)
          {
            ...
            for (int h = 0; h < int.Parse(Rows[Pos + 1]); h++, TempH++)
              StudMarksMatrix[l, h] = Rows[Pos + 2 + h];
            Pos += TempH + 1;
          }
          LessonType.Add(new LessonMarks(LessonName, Dates, StudMarksMatrix, MiddleMarks, IsWas));
        }
        StudMarks.Add(new MarksForClass(ClassName, LessonType));
        Pos += 2;
      }
      return new Journal(Subjects, Groups, StudMarks);
    }
    ...
    public static List<Group> GetGroupList(string InputText)
    {
    ...
    public string Save()
    {
      string Out = Convert.ToString(StudMarks.Count) + "\n";
      for (int i = 0; i < StudMarks.Count; i++)
      {
        Out += StudMarks[i].ClassName + "\n";
        Out += StudMarks[i].Marks.Count + "\n";
        for (int k = 0; k < StudMarks[i].Marks.Count; k++)
        {
          Out += StudMarks[i].Marks[k].LessonName + "\n";
          Out += StudMarks[i].Marks[k].Dates.Count + "\n";
          for (int j = 0; j < StudMarks[i].Marks[k].Dates.Count; j++)
          {
            Out += StudMarks[i].Marks[k].Dates[j] + "\n";
            Out += StudMarks[i].Marks[k].IsWas[j] + "\n";
          }
          Out += StudMarks[i].Marks[k].MiddleMarks.Length + "\n";
          for (int j = 0; j < StudMarks[i].Marks[k].MiddleMarks.Length; j++)
            Out += StudMarks[i].Marks[k].MiddleMarks[j] + "\n";
          Out += StudMarks[i].Marks[k].MarksMatrix.GetLength(0) + "\n";
          for (int j = 0; j < StudMarks[i].Marks[k].MarksMatrix.GetLength(0); j++)
          {
            Out += StudMarks[i].Marks[k].MarksMatrix.GetLength(1) + "\n";
            for (int l = 0; l < StudMarks[i].Marks[k].MarksMatrix.GetLength(1); l++)
              Out += StudMarks[i].Marks[k].MarksMatrix[j, l] + "\n";
          }
        }
      }

      string Less = Convert.ToString(Lessons.Count);
      for (int i = 0; i < Lessons.Count; i++)
      {

      }

      string Studs = Convert.ToString(Groups.Count);
      for (int i = 0; i < Groups.Count; i++)
      {
        Studs += Groups[i].GroupName + '\n';
        Studs += Groups[i].StudNames.Length + '\n';
        for (int k = 0; k < Groups[i].StudNames.Length; k++)
          Studs += Groups[i].StudNames[i] + '\n';
      }

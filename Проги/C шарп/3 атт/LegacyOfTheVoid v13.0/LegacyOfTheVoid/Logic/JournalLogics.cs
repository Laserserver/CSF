...
namespace ProjectRed
{
  class JournalLogics
  {
    public static void Parse(string Input, out List<Group> Groups, out List<ClassSubject> Subjects, out List<ClassMarks> ClassMarks)
    {
      string[] New = Input.Split(';');

      Groups = GetGroupList(New[0]);
      Subjects = GetSubjectsPlan(New[1]);
      ClassMarks = new List<ClassMarks>();
    ...
    public static string Save(List<Group> Groups, List<ClassMarks> ClassMarks, List<ClassSubject> ClassSubjects)
    {
      string Studs = null;
